...
(defvar vimrc-use-smie t
  "When non-nil use `smie' for indentation.")

(defcustom vimrc-indent-offset 2
  "Default indentation level to use in `vimrc-mode'."
  :group 'vimrc-mode
  :type 'integer)
...
(defun vimrc-smie-rules (kind token)
  (pcase (cons kind token)
    (`(:elem . basic) vimrc-indent-offset)
    (`(:elem . args) 0)
    (`(:close-all . ,_) t)
    (`(:list-intro . ,(or `"\n" `"")))
    ))
...
(define-derived-mode vimrc-mode vimrc--parent-mode "Vimrc"
  ...
  ;; indentation
  (when vimrc-use-smie
    (smie-setup vimrc-smie-grammar #'vimrc-smie-rules
